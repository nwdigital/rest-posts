...
class RESTposts_widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		$siteurl = ( ! empty( $instance['siteurl'] ) ) ? ( $instance['siteurl'] ) : site_url();
			if ( ! $siteurl )
				$siteurl = site_url();
		$post_type = ( !empty( $instance['post_type'] ) ) ? $instance['post_type'] : 'posts';
		...
		$valid_url = wp_remote_get( ( $siteurl ) . '/wp-json/wp/v2/'.$post_type.'/?per_page=1' );
        ...
        <input class="widefat" id="<?php echo $this->get_field_id( 'siteurl' ); ?>" name="<?php echo $this->get_field_name( 'siteurl' ); ?>" type="text" value="<?php echo esc_attr( $siteurl ); ?>" /><?php echo $url_checkmark; ?></p>
				...
				<p><label for="<?php echo $this->get_field_name( 'post_type' ); ?>">Post Type (default 'posts' or enter custom post type): </label>
	...
	$response = wp_remote_get( $instance['siteurl'] . '/wp-json/wp/v2/'.$instance['post_type'].'/?per_page=' . $instance['numpost'] );
	...
	if( !empty( $posts ) ) {
		...
		$the_excerpt = strip_tags($post->excerpt->rendered);
			...
			if( isset( $instance['showtitle'] ) && !empty($post->title->rendered) ) : ?>
				<?php echo $post_link; ?><div class="mrp-post-title"><h4><?php echo $post->title->rendered; ?></h4></div></a>
			<?php endif;
			if( isset( $instance['featimg'] ) && !empty($post->swp_thumbnail) ) : ?>
				<?php echo $post_link; ?><div class="mrp-feat-img"><img src ="<?php echo esc_url($post->swp_thumbnail); ?>"/></div></a>
			<?php endif;
			if( isset( $instance['excerpt'] ) && !empty($excerpt) ) : ?>
				<div class="mrp-widget-excerpt"><?php echo '<span class="mrp-excerpt">' . $excerpt . '</span>'; ?></div>
			<?php endif;
			if( isset( $instance['showdate'] ) && !empty($humandate) ) : ?>
